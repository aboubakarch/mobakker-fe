...
import { employeeRatingColumns } from './columns/EmployeeRatingColumn';
// import EmployeeRatingFilters from './filters/EmployeeRatingFilters';

const EmployeeRatingTable: FC<ITableProps<SampleEmployeeRating>> = ({
    handleEdit, handleDelete, onUpdateFlag, handleRow
}) => {
    const { t } = useTranslation();
    const { toast } = useToast();
    const [data, setData] = useState<SampleEmployeeRating[]>([]);
    ...
    // const [filters, setFilters] = useState<IEmployeeRatingFilters>({});
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await APIService.getInstance().getEmployeeRatings(params);
    ...
    // const handleApplyFilters = (fil: IEmployeeRatingFilters) => {
    //     setFilters(fil);
    // };
    ...
    return (
        <div>
            {!pageLoaded && data.length === 0 ? (
                ...
                <DataTable
                    data={data}
                    columns={employeeRatingColumns(t)}
                    filterKey='rating'
                    count={total}
