...
import { customerRatingColumns } from './columns/CustomerRatingColumn';
// import CustomerRatingFilters from './filters/CustomerRatingFilters';

const CustomerRatingTable: FC<ITableProps<SampleCustomerRating>> = ({
    handleEdit, handleDelete, onUpdateFlag, handleRow
}) => {
    const { t } = useTranslation();
    const { toast } = useToast();
    const [data, setData] = useState<SampleCustomerRating[]>([]);
    ...
    // const [filters, setFilters] = useState<ICustomerRatingFilters>({});
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await APIService.getInstance().getCustomerRatings(params);
    ...
    // const handleApplyFilters = (fil: ICustomerRatingFilters) => {
    //     setFilters(fil);
    // };
